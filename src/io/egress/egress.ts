...
import { createComparator, heapPush, heapReplaceRoot } from "@/core/engine/compare";
...
export class EgressEngine<T extends Record<string, unknown>> {
    private constructor(private readonly state: EgressState<T>) {}

    static from<T extends Record<string, unknown>>(ingress: IngressEngine<T>, plan: QueryPlan<T>): EgressEngine<T> {
        return new EgressEngine<T>({
            ingress,
            plan,
            orders: [],
            limitCount: null,
    ...
    private clone(next: Partial<EgressState<T>>): EgressEngine<T> {
        return new EgressEngine<T>({ ...this.state, ...next });
    }

    orderBy<P extends SortablePaths<T>>(field: P, options?: OrderOptions): EgressEngine<T> {
    ...
    orderByDate<P extends DatePaths<T>>(field: P, options?: OrderOptions): EgressEngine<T> {
        const segments = getSegments(this.state.plan.cache, String(field));
        const direction = options?.direction === "desc" ? -1 : 1;
        const nullsFirst = options?.nulls === "first";
        const order: OrderSpec = {
    ...
    thenBy<P extends SortablePaths<T>>(field: P, options?: OrderOptions): EgressEngine<T> {
        return this.orderBy(field, options);
    }

    thenByDate<P extends DatePaths<T>>(field: P, options?: OrderOptions): EgressEngine<T> {
        return this.orderByDate(field, options);
    }

    limit(count: number): EgressEngine<T> {
        const safeCount = Number.isFinite(count) ? Math.max(0, Math.floor(count)) : 0;
        return this.clone({ limitCount: safeCount });
    }

    offset(count: number): EgressEngine<T> {
        const safeCount = Number.isFinite(count) ? Math.max(0, Math.floor(count)) : 0;
        return this.clone({ offsetCount: safeCount });
    }

    page(page: number, pageSize: number): EgressEngine<T> {
    ...
    result(): T[] {
        ...
        const filtered = executor.execute(this.state.ingress, this.state.plan);
