...
export function sqliteTableSource<T extends Record<string, unknown>>(
    ...
    const stream = (_options?: import("@/core/aot/features/prefilter").PrefilterStreamOptions) => streamSqlite<T>({
        db: options.db,
        params: undefined,
...
export function sqliteQuerySource<T extends Record<string, unknown>>(
    ...
    const stream = (_options?: import("@/core/aot/features/prefilter").PrefilterStreamOptions) => streamSqlite<T>(options);
    const materialize = async () => {
        const items: Array<T> = [];
        for await (const item of stream()) {items.push(item);}
        return items as ReadonlyArray<T>;
